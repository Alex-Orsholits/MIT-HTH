...
AFRAME.registerComponent("vpt-stream", {
  ...
  init: function () {
    ...
    this.vptstream.addEventListener(
  ...
  tick: function (t) {
    ...
    if (this.vptstream.LoadTime > 0 && dT > this.data.uiDelay) {
      ...
      if (this.vptstream.playing) {
        if (uiVisible) {
          this.el.sceneEl.emit("hide_autoplay_dialog", {
            videoref: this.vptstream.video,
          });
          this.autoplayUi.setAttribute("visible", false);
        }
      } else {
        if (!uiVisible) {
          this.el.sceneEl.emit("show_autoplay_dialog", {
            videoref: this.vptstream.video,
          });
  ...
  async loadMedia() {
    ...
    const proxySrc = "https://mit.kenchitaru.studio/hls/playlist.m3u8";
